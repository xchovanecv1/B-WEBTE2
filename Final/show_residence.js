...
function geocodeAddress(user) {
    console.log(user);
    addRunnerPositionMarker(user.geo, user);
}
...
function test_residence_API() {
    ...
    $.post("./gateway.php", data).done(function (data) {
        ...
        for(var i = 0; i<response["users"].length; i++) {
            var dt = response["users"][i];
            var jsn = JSON.parse(dt['geo']);
            dt['geo'] = jsn;
            geocodeAddress(response["users"][i]);
        }
    });
}
...
function addRunnerPositionMarker(location, user) {
    ...
    var marker = new google.maps.Marker({
        ...
        name: contentString,
        icon: {
            url: "https://mt.google.com/vt/icon/name=icons/spotlight/home_L_8x.png&scale=1.15",
        },
        map: map
    });
