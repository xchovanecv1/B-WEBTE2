...
function showRouteAndUserPosition(inp, distance){
    ...
    directionsService.route({
        ...
        function(response, status) {
            ...
            var location  = getLatLngOnRoute(response, distance);

            //vykresli poziciu bezca na trase
            addRunnerPositionMarker(location, map, distance);
        });
}
...
function test_user_API()
{
    ...
    $.post("./gateway.php", data).done(function(data){
        ...
        let response = jQuery.parseJSON(data);
        console.log(response);
        showRouteAndUserPosition(JSON.parse(response["description"]),response["vzdialenost"])
    });
}
...
function addRunnerPositionMarker(location, map, distance) {


    var contentString =
        "<h5>Prejdená vzdialenosť: </h5>" + distance;

    var marker = new google.maps.Marker({
        position: location,
        name: contentString,
        icon: {
            url: "https://raw.githubusercontent.com/Concept211/Google-Maps-Markers/master/images/marker_greenR.png",
        },
        map: map
    });
